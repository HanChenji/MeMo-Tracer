...
void Decoder::emitDiv(Instr& instr, DynUopVec& uops) {
    ...
    //assert(dsts == 3);
    //assert(instr.numInRegs > 1); //there is divb: Unsigned divide AX by r/m8, with result stored in AL := Quotient, AH := Remainder.
    ...
    if (srcs == 3 && dsts == 3) {
        ...
        emitStores(instr, uops);
    } else if (srcs==2 && dsts==2){/* for insts of divb */
        emitLoads(instr, uops);

        emitExecUop(srcRegs[0], srcRegs[1], REG_EXEC_TEMP, 0, uops, lat, PORTS_015, extraSlots);
        emitExecUop(REG_EXEC_TEMP, 0, dstRegs[0], dstRegs[1], uops, 1, PORTS_015); //quotient and remainder
...
BblInfo* Decoder::decodeBbl(BBL bbl, bool oooDecoding) {
    uint64_t bbl_addr = BBL_Address(bbl);
    uint32_t instrs = BBL_NumIns(bbl);
    uint32_t bytes = BBL_Size(bbl);
    BblInfo* bblInfo;

    // check if the bbl is already decoded
    BasicBlockKey bblKey(bbl_addr, instrs);
    auto bi = bbl_map.find(bblKey);
    if (bi != bbl_map.end()) {
        return bi->second;
    }
    ...
    if (oooDecoding) {
        ...
        bblInfo = static_cast<BblInfo*>(gm_malloc(offsetof(BblInfo, oooBbl)));
    }
    ...
    bblInfo->bblIdx = bblIdx++;
    futex_unlock(&bblIdxLock);
    assert(bblIdx < MAX_BBLS);

    bbl_map.insert({bblKey, bblInfo});

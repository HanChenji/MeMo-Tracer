...
PinCmd::PinCmd(Config* conf, const char* configFile, uint64_t shmid) {
    ...
    //Global pin options
    //Child injection creates the application process as a child of the pin process so you will see both a pin process and the application process running.
#ifndef DEBUG_ZSIM
...
std::vector<std::string> split_redirections(const std::string &s) {
    std::vector<std::string> tokens;
    size_t start = 0;
    bool find = false;
    // delimiter candidates
    std::vector<std::string> delimiters = {"<", ">", "2>", "&>", ">>", "2>>", "&>>"};
    ...
    size_t end;
    if (find) {
        end = *std::min_element(ends.begin(), ends.end());
    } else {
        end = s.length();
    }

    tokens.push_back(s.substr(start, end - start));
    tokens.push_back(s.substr(end));
    return tokens;
}

inline g_string drop_redirections(const g_string &s) {
    std::vector<std::string> tokens = split_redirections(s.c_str());

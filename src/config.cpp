...
void Config::writeAndClose(const char* outFile, bool strictCheck) {
    ...
    if (unused) {
        if (strictCheck) {
            fprintf(stderr, "%d setting%s not used during configuration\n", unused, (unused > 1)? "s" : "");
...
template<> int      Config::get<int>(const char* key) {return (int) genericGet<int>(key);}
...
template<> int      Config::get<int>(const char* key, int def) {return (int) genericGet<int>(key, (int)def);}

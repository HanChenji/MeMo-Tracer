...
class CC : public GlobAlloc {
    public:
        //Initialization
        virtual void setParents(uint32_t childId, const g_vector<MemObject*>& parents, Network* network=nullptr) = 0;
        virtual void setChildren(const g_vector<BaseCache*>& children, Network* network=nullptr) = 0;
...
class MESICC : public CC {
    ...
    public:
        ...
        void setParents(uint32_t childId, const g_vector<MemObject*>& parents, Network* network=nullptr) {
            bcc = new MESIBottomCC(numLines, childId, nonInclusiveHack);
            bcc->init(parents, network, name.c_str());
        }

        void setChildren(const g_vector<BaseCache*>& children, Network* network=nullptr) {
